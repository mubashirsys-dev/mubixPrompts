...
import { getSmartFeatures, featureCategoryLabels } from "@/lib/features";
...
export function FeaturesStep() {
  const { selectedCategory, selectedFeatures, toggleFeature, setSelectedFeatures, nextStep, prevStep } = useBuilderStore();

  const smartFeatures = getSmartFeatures(selectedCategory?.id);
    ...
    <div className="space-y-8 bg-white border-4 border-black p-6 sm:p-8 shadow-[8px_8px_0px_0px_#000]">
      <div className="text-center mb-8">
        <span className="inline-block px-3 py-1 bg-[#FFD93D] border-2 border-black font-black uppercase text-xs rotate-[-1deg] mb-4">
          STEP 5: SMART FEATURE ENGINE
        </span>
        <h2 className="text-2xl font-black uppercase text-black">Select features for your project</h2>
        <p className="text-sm font-bold text-black/70 mt-2">
          Only displaying highly-relevant features for your selected category: <strong className="underline uppercase">{selectedCategory?.name}</strong>.
      ...
      <div className="space-y-10 max-w-4xl mx-auto">
        {categoryOrder.map((category) => {
          const catFeatures = smartFeatures.filter((f) => f.category === category);
          if (catFeatures.length === 0) return null; // Hide categories with no matching smart features
